...
class Pemakaian_model extends CI_Model
{
    ...
    public function generateID()
    {
        ...
        $query = $this->db->order_by('ID_PINJAM', 'desc')->limit(1)->get('pemakaian')->row('ID_PINJAM');
    ...
    {
        $query = $this->db->where('QTY != 0')->get('pc')->result();
    ...
    {
        $hm = $this->db->where('ID_PC', $kode)->get('pc');
        return $hm;
    }
    ...
    public function insert()
    {
        ...
        $this->db->insert('pemakaian', $pinjam);

        for ($i = 1; $i <= $pc; $i++) {
            $kd_pc = $this->input->post('slcBookCode' . $i);
            $detail = array(
                'ID_DIPINJAM'   => NULL,
                'ID_PINJAM'     => $this->input->post('tbBrwCode'),
                'ID_PC'       => $kd_pc,
            ...
            $qty = $this->db->where('ID_PC', $kd_pc)
                ->get('pc')
                ->row('QTY');
            $min = (int) $qty - 1;
            $this->db->set('QTY', $min)->where('ID_PC', $kd_pc)->update('pc');

            //Menambah kolom di pinjam
            $keluar = $this->db->where('ID_PC', $kd_pc)
                ->get('pc')
                ->row('KELUAR');
            $plus = (int) $keluar + 1;
            $this->db->set('KELUAR', $plus)->where('ID_PC', $kd_pc)->update('pc');
        }
